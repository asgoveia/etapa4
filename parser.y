...
decl:   
          type TK_IDENTIFIER '(' parameterList ')' block       {$$=astreeCreate(AST_FUNC, $2, $1, $4, $6, 0, getLineNumber());}    
...
param: type TK_IDENTIFIER                  {$$=astreeCreate(AST_PARAM, $2, $1, 0, 0, 0, getLineNumber());}
       ;

parameterList:    
           param rest         {$$=astreeCreate(AST_LPARAM,0,$1,$2,0,0, getLineNumber());}
        |                     {$$=0;}
        ;

rest:       
           ',' param rest                  {$$=astreeCreate(AST_RPARAM, 0, $2, $3, 0, 0, getLineNumber());}
        |                                  {$$=0;}
        ;
...
expParamRest:       
           ',' exp expParamRest                {$$=astreeCreate(AST_EPARAMREST, 0, $2, $3, 0, 0, getLineNumber());}
        |                                      {$$=0;}
        ;
...
exp:        
        ...
        |  '~' exp                             {$$=astreeCreate(AST_NOT, 0, $2, 0, 0, 0, getLineNumber());}
